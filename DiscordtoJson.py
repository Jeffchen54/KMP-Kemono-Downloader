...
class DiscordToJson():
    ...
    def discord_channel_lookup(self, channelID:str|None, scraper:CloudflareScraper)->dict:
        """
        ...
        if(not channelID or channelID == self.recent.get("channelID")):
            ...
            self.recent = {"channelID" : self.recent.get("channelID"), "skip" : skip + SKIP}
            channelID = self.recent.get("channelID")

        # If history exists but does not match, start from beginning
        else:
            skip = 0
            self.recent = {"channelID" : channelID, "skip" : skip + SKIP}
